...
// 主要部材のみ2桁（上部=前2桁 / 下部=後2桁）、それ以外は'00'。※main.pyのbuzai_no_toggleと同一ルール
const _TOGGLE_UPPER = new Set(['Mg','Gb','Cr','St','Bt','Pp','Em','Ar','Sa','Rg','Rp','Ts','Cn']);
const _TOGGLE_LOWER = new Set(['Pw','Pb','Pc','Ap','Ac','Aw','Ff']);
function _buzaiNo(symbol, elementNo) {
  const sym = (symbol || '').trim();
  const en = (elementNo || '').trim();
  let head;
  if (_TOGGLE_UPPER.has(sym)) head = true;
  else if (_TOGGLE_LOWER.has(sym)) head = false;
  else return '00';
  if (/^\d{4}$/.test(en)) return head ? en.slice(0, 2) : en.slice(2);
  if (/^\d{2}$/.test(en)) return en;
  return '00';
}
// 部材行テキスト（"Mg0101" や "Mg0101→0102"）の識別番号部分だけを部材番号へ置換
function _memberToBuzai(memberLine) {
  if (!memberLine) return memberLine;
  const arrow = memberLine.replace(/([A-Z][a-z]+)(\d+)→(\d+)/, (m, sym, a, b) => sym + _buzaiNo(sym, b.padStart(4, '0')));
  if (arrow !== memberLine) return arrow;
  return memberLine.replace(/([A-Z][a-z]+)(\d+)/, (m, sym, no) => sym + _buzaiNo(sym, no.padStart(4, '0')));
}
...
export function exportDxf({ rawText, dxfFileName, damagesData, numberingLabel, renderData, buzaiMode = false }) {
  if (!rawText) {
          ...
          while (j < resultLines.length) {
            ...
            if (resultLines[j]?.trim() === '1' && j + 1 < resultLines.length) {
              ...
              if (sep !== -1) {
                ...
                const finalMember = buzaiMode ? _memberToBuzai(memberPart) : memberPart;
                resultLines[j + 1] = finalMember + '\\P' + finalDmg;
              }
              break;
            }
            j += 2;
  ...
  for (const { d, raw } of textPairNewMtext) {
    ...
    const content = (buzaiMode ? _memberToBuzai(raw.memberLine) : raw.memberLine) + '\\P' + finalDmg;
  ...
  if (addedDamages.length > 0) {
    ...
    addedDamages.forEach((d, idx) => {
      ...
      const _elNoBz = buzaiMode ? _buzaiNo(d.symbol || '', (d.elementNoCurrent || '').padStart(4, '0')) : (d.elementNoCurrent || '');
      const memberPart = (d.symbol || '') + _elNoBz + (d.memberName ? ' ' + d.memberName : '');
